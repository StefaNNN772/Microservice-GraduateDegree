...
namespace RouteService.Repository
{
    public class BusLinesRepository
    {
        ...
        {
            return await _context.BusLines.Include(b => b.Schedule).Where(b => b.ScheduleId == id).ToListAsync();
        }
        ...
        public async Task<bool> UpdateBusLine(long id, int numberOfSeats)
        {
            var busLine = await _context.BusLines.Where(b => b.Id == id).FirstOrDefaultAsync();

            bool result = false;

            if (busLine.AvailableSeats != null)
            {
                busLine.AvailableSeats -= numberOfSeats;
