...
namespace ReservationService.Controllers
{
    ...
    public class BusReservationController : ControllerBase
    {
        ...
        [HttpPost("busReservation/getTickets/{scheduleId}")]
        [Produces("application/json")]
        public async Task<ActionResult<List<TicketDTO>>> GetTickets(long scheduleId)
        {
            var ticketsDTO = await _busReservationService.UserToNotify(scheduleId);
        ...
        [HttpPost("busReservation/getTicketsForUpdate/{scheduleId}")]
        [Produces("application/json")]
        public async Task<ActionResult<List<TicketDTO>>> GetTicketsForUpdate(long scheduleId)
        {
            var ticketsDTO = await _busReservationService.GetTicketsToNotifyForUpdate(scheduleId);
        ...
        public async Task<IActionResult> AddReservation([FromBody] ReservationRequestDTO data)
        {
            ...
            Console.WriteLine(busSeats.Id);
            Console.WriteLine(busSeats.Schedule.DepartureTime);

            if (busSeats == null)
            {
                return StatusCode(500, "Failed to retrieve bus line information.");
            }

            if (busSeats.Schedule == null)
            {
                return StatusCode(500, "Bus line schedule information is missing.");
            }
        ...
        {
            Console.WriteLine(busLine.Schedule.Departure);
            try
            {
